-- Migration 012: Enable Parent Access to Player Data (FIXED VERSION)
-- Feature Slice 6: Parent Access & Privacy
-- This migration updates RLS policies to allow parents to view their children's data
-- Uses DROP POLICY IF EXISTS to avoid "already exists" errors
...
DROP POLICY IF EXISTS "Parents can view their children" ON public.players;
...
-- NOTE: We avoid joining match_players to prevent infinite recursion
...
CREATE POLICY "Parents can view matches their children participated in"
    ...
    USING (
        -- Parents can view matches for teams where their children play
        EXISTS (
            SELECT 1
...
DROP POLICY IF EXISTS "Parents can view their children's match participation" ON public.match_players;
...
DROP POLICY IF EXISTS "Parents can view their children's match events" ON public.match_events;
...
DROP POLICY IF EXISTS "Parents can view their children's awards" ON public.match_awards;
...
DROP POLICY IF EXISTS "Parents can view period tracking for their children's matches" ON public.period_tracking;
