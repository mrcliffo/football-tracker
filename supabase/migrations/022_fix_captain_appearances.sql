-- Migration 022: Fix captain appearances calculation
-- The captain_appearances was incorrectly looking at matches.captain_id
-- It should count match_players where is_captain = true

-- Drop and recreate the view with correct captain calculation
...
SELECT
    ...
    -- Captain appearances (count matches where player was captain)
    COUNT(DISTINCT CASE WHEN mp.is_captain = true THEN mp.match_id END) AS captain_appearances
FROM
    public.players p
    LEFT JOIN public.match_players mp ON p.id = mp.player_id
    LEFT JOIN public.matches m ON mp.match_id = m.id AND m.is_active = true AND m.status = 'completed'
...
-- Add comment
COMMENT ON VIEW public.player_stats_view IS 'Aggregated player statistics including goals, assists, tackles, saves, cards, and awards - Fixed captain appearances';
